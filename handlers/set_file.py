...
async def set_file_id(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer('Отправьте фото, видео или гиф')
...
async def set_file(message: Message, state: FSMContext):
    ...
    if photo:
        await message.answer_photo(file_id, caption='✅ Фото успешно добавлено!', reply_markup=del_file_keyboard())
    elif video:
        await message.answer_video(file_id, caption='✅ Видео успешно добавлено!', reply_markup=del_file_keyboard())
    elif animation:
        await message.answer_animation(file_id, caption='✅ Гифка успешно добавлена!', reply_markup=del_file_keyboard())
